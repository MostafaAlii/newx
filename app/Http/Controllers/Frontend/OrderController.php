...
//    protected $twoCheckoutService;
//
//    public function __construct(TwoCheckoutService $twoCheckoutService)
//    {
...
//                 Referance image
    ...
    public function proceedCheckoutPayment($order)
    {
        ...
        $newOrder = Order::findorfail($order->id);
        $total = OrderDetails::where('order_id', $order->id)->sum('total');

        $params = array(
            'sid' => '1659666',
            'mode' => '2CO',
            'li_0_name' => $order->slug,
            'li_0_price' => $total,
            'card_holder_name'=>$newOrder->user->name,
...
//        );
//
//        $checkout = new TwoCheckoutService();
//
//         $checkout->pay($params);
